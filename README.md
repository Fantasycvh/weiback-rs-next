...
- Windows 发布、签名、候选验证、升级/并存/卸载与备份回滚流程见 [发布与安装完整性](docs/release.md)。
...
- 在“全局数据维护”中可创建用户数据备份。备份使用 SQLite `VACUUM INTO` 一致性快照，并仅包含数据库记录中已下载且仍被引用的媒体文件；旧版图片和视频记录也会按配置的存储根收集。
- 备份写入应用数据目录的 `imports/backups/`。每个完整批次均含版本化清单、相对路径、长度和 SHA-256；`.part`、导入暂存文件及孤儿媒体不会被纳入。
- 恢复前会校验清单路径、哈希与 SQLite 完整性并完成暂存预检，校验失败不会修改当前数据。恢复会先生成当前数据库的 rollback 快照，并替换数据库、统一媒体根及配置在外部的旧版图片/视频根；进行中的普通任务或写操作会被拒绝。成功后应用自动退出，重新启动后会恢复 pending 媒体及持久任务。
- 备份只包含数据库和媒体，**不包含登录 session/token**。
